...
export class BrowserResponse implements Response {
  #includePages = false;
  ...
  async handle(
    ...
    if (this.#listInPageTools) {
      inPageTools = await getToolGroup(context.getSelectedBrowserPage());
    }

    let consoleMessages: Array<ConsoleFormatter | IssueFormatter> | undefined;
    if (this.#consoleDataOptions?.include) {
      if (!this.#page) {
        throw new Error(`Response must have a BrowserPage`);
      }
...
function createStructuredPage(page: Page, context: BrowserContext) {
  const isolatedContextName = context.getIsolatedContextName(page);
  const entry: {
    id: number | undefined;
    url: string;
